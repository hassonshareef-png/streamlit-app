...
# Auth - passwords SHA-256 hashed. Default password: NexaDash2026!
USERS = {
...
st.sidebar.markdown('---')
st.sidebar.markdown(f'**{st.session_state.username}** - Pro Plan')
...
if page == 'Home':
    ...
    fig.update_layout(paper_bgcolor='#1e2130', plot_bgcolor='#1e2130',
                                            font=dict(color='#aaa'), margin=dict(l=20, r=20, t=20, b=20))
    ...
    with L:
                ...
                rev = np.cumsum(np.random.randint(2000, 5000, 30)) + 40000
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=days, y=rev, fill='tozeroy', name='Revenue',
                                         line=dict(color='#7c83fd', width=2),
                                         fillcolor='rgba(124,131,253,0.15)'))
                fig.add_trace(go.Scatter(x=days, y=np.linspace(rev[0], rev[-1]*1.1, 30),
                                         name='Target', line=dict(color='#3fb950', dash='dash')))
                fig.update_layout(paper_bgcolor='#1e2130', plot_bgcolor='#1e2130',
                                  font=dict(color='#aaa'), margin=dict(l=20, r=20, t=20, b=20),
            ...
            with R:
                        ...
                        fig2 = go.Figure(go.Pie(
                            ...
                            values=[38, 27, 15, 12, 8], hole=0.55,
                            marker=dict(colors=['#7c83fd','#3fb950','#f78166','#e3b300','#79c0ff'])))
                        fig2.update_layout(paper_bgcolor='#1e2130', font=dict(color='#aaa'),
    ...
    with c1:
            ...
            with c2:
                        ...
                        fig2 = go.Figure(go.Scatter(
                            x=['Wk1','Wk2','Wk3','Wk4'], y=[100, 68, 52, 43],
                            mode='lines+markers', line=dict(color='#3fb950', width=3), marker=dict(size=10)))
    ...
    st.title('Settings')
    with st.form('s'):
                st.text_input('Name', value=st.session_state.username.capitalize())
