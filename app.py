...
if page == 'Home':
        ...
        with L:
            ...
            st.subheader('Traffic Sources')
                            fig2 = go.Figure(go.Pie(labels=['Organic','Paid','Referral','Social','Direct'],values=[38,27,15,12,8],hole=0.55,marker=dict(colors=['#7c83fd','#3fb950','#f78166','#e3b341','#58a6ff'])))
        fig2.update_layout(paper_bgcolor='#1e2130',font=dict(color='#aaa'),margin=dict(l=10,r=10,t=10,b=10))
        st.plotly_chart(fig2, use_container_width=True)
    st.subheader('Recent Transactions')
    st.dataframe(pd.DataFrame({'Date':['May 10','May 9','May 9','May 8','May 7'],'User':['Alice M.','Bob K.','Carol T.','Dan R.','Eve S.'],'Plan':['Pro','Team','Pro','Starter','Team'],'Amount':['$49','$149','$49','$19','$149'],'Status':['Paid','Paid','Paid','Pending','Paid']}), use_container_width=True)
